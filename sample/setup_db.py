...
def create_table_if_not_exists(conn, table_name, df, primary_key=None, foreign_key: dict = {},long_text=True):
    ...
    for col in df.columns:
        if pd.api.types.is_integer_dtype(df[col]):
            ...
            col_type.append('FLOAT')
        elif pd.api.types.is_object_dtype(df[col]) and long_text:
            col_type.append('TEXT')  # Use TEXT for long text if enabled
        else:
            col_type.append('VARCHAR(255)')
...
if __name__ == '__main__':
    ...
    load_csv_to_postgres(
        ...
        port=port,
        # primary_key=primary_key_sub_and_shareholder,
        # foreign_key=foreign_key_sub_and_shareholder
    )
